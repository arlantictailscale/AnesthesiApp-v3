...
export function CBTDiscussion({ packageId, packageName }: CBTDiscussionProps) {
  const [user, setUser] = useState<any>(null)
  
  // Ratings state
  const [ratings, setRatings] = useState<CBTRatingsSummary>({ average: 0, count: 0 })
  const [hoverRating, setHoverRating] = useState<number | null>(null)
  const [ratingLoading, setRatingLoading] = useState(false)

  // Comments state
  const [comments, setComments] = useState<CBTComment[]>([])
  ...
  async function handleRate(value: number) {
    ...
    try {
      await ratePackage(packageId, value)
        ...
        <CardContent className="space-y-4">
          ...
          {user ? (
            ...
            <div className="p-3 bg-muted/30 border border-border rounded-lg text-center text-xs text-muted-foreground">
              Silakan login untuk bergabung dalam ruang diskusi paket soal ini.
            </div>
          )}
